...
Each kernel startup loads Pyodide wheels for **`numpy`** and **`matplotlib`**. `plt.show()` renders **inline PNGs** under the cell (Agg backend), not floating browser windows.
