...
The Pyodide kernel preloads **numpy** and **matplotlib** (first run downloads wheels from the Pyodide CDN). Use `import numpy as np` and `import matplotlib.pyplot as plt`; `plt.show()` uses the in-browser canvas backend.
